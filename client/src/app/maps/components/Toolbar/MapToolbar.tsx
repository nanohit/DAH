...
interface MapToolbarProps {
  ...
  onBlurMapName: () => Promise<void> | void;
  onSave: () => Promise<void>;
  ...
  mapNameInputRef: React.RefObject<HTMLInputElement>;
  ...
  return (
    <div className="absolute top-5 left-5 bg-white rounded-lg shadow-lg px-3 p-2 flex items-center gap-1 z-20 h-14">
      ...
      <div className="flex items-center relative">
        ...
        {isAutosaveEnabled && (
          <span className={`text-xs ml-2 ${isAutoSaving ? 'text-yellow-600' : 'text-green-600'} flex items-center`}>
            {isAutoSaving ? (
      ...
      <button onClick={onSave} className="p-2 hover:bg-gray-100 rounded-lg text-gray-700 flex items-center justify-center transition-colors" title="Save">
        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="relative">
        <button className="p-2 hover:bg-gray-100 rounded-lg text-gray-700 transition-colors" title="More options" onClick={() => setShowDropdown(!showDropdown)}>
        ...
        {showDropdown && (
          <div ref={dropdownRef} className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
            ...
            <div className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm text-gray-700 flex items-center" onClick={onCopyShareLink}>
              {linkCopied ? (
                <>
                ...
                {showDeleteConfirm ? (
                  <div className="px-4 py-2 text-sm text-red-600">
                    ...
                    <div className="flex justify-between">
                      <button className="px-2 py-1 text-xs bg-red-600 text-white rounded" onClick={onDeleteMap}>
                        Delete
                      </button>
                      <button className="px-2 py-1 text-xs bg-gray-200 text-gray-800 rounded" onClick={() => setShowDeleteConfirm(false)}>
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm text-red-600 flex items-center" onClick={() => setShowDeleteConfirm(true)}>
                    Delete map
                  </div>
                )}
              </>
