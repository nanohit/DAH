...
import { Editor, TLPageId, getIndexBetween } from 'tldraw';
...
const TLMapToolbar = ({
  ...
  const [pages, setPages] = useState<{ id: TLPageId; name: string; index: string }[]>([]);
  ...
  const refreshPages = useCallback(() => {
    if (!editor) return;
    const all = editor.getPages().map((p) => ({ id: p.id as TLPageId, name: p.name, index: p.index }));
    setPages(all);
  ...
  const handleSwitchPage = (id: TLPageId) => {
    if (!editor) return;
  ...
  const handleAddPage = () => {
    ...
    if (created) {
      const newId = created.id as TLPageId;
      editor.setCurrentPage(newId);
      setCurrentPageId(newId);
      setPages([...after.map((p) => ({ id: p.id as TLPageId, name: p.name, index: p.index }))]);
    }
    setShowPagesMenu(false);
    setTimeout(() => refreshPages(), 0);
  };

  // Keep current page name in sync with map name when appropriate
  useEffect(() => {
    if (!editor) return;
    const currentId = editor.getCurrentPageId();
    const page = editor.getPage(currentId);
    if (!page) return;
    if (page.name === 'Page 1' || page.name === mapName) {
      editor.updatePage({ id: page.id, name: mapName || 'Untitled' });
      refreshPages();
    }
  }, [mapName, editor, refreshPages]);
  ...
  return (
    <>
      <div className="absolute top-5 left-5 bg-white rounded-lg shadow-lg px-2 py-1.5 flex items-center gap-0.5 z-[300]">
        {/* Back button removed in local-only mode */}
        ...
        <div className="flex items-center relative">
          <input
            ...
            className="text-gray-800 text-sm font-medium bg-transparent border border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none rounded px-1 py-0.5 transition-colors"
            placeholder="Untitled"
          />
          <span
            ref={hiddenTextRef}
            className="absolute opacity-0 pointer-events-none text-sm font-medium"
        ...
        {onSave && (
          <button
            ...
            disabled={isSaving}
            aria-label="Save map"
            className={`ml-1 p-2 text-xs font-medium rounded transition-colors ${
              isSaving 
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed' 
                : 'text-gray-700 hover:bg-gray-100'
            }`}
            title="Save map to server"
          >
            {isSaving ? (
        ...
        {onSetPrivacy && (
          <button
            onClick={() => onSetPrivacy(!isPrivate)}
            className={`ml-1 p-1.5 rounded-lg transition-colors ${
              isPrivate 
                ? 'bg-blue-100 text-blue-600 hover:bg-blue-200' 
                : 'text-gray-500 hover:bg-gray-100'
            }`}
            title={isPrivate ? 'Map is private (click to make public)' : 'Map is public (click to make private)'}
          >
            {isPrivate ? (
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="relative">
          <button
            ...
            className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-700 flex items-center justify-center transition-colors"
            title="Pages"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="m6 9 6 6 6-6" />
          ...
          {showPagesMenu && (
            <div className="absolute left-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
              <div className="px-3 py-2 text-xs font-medium text-gray-500">Pages</div>
              <div className="max-h-60 overflow-auto">
                {pages.map((p, idx) => (
                  <div
                    ...
                    draggable
                    onDragStart={(e) => {
                      e.dataTransfer.setData('text/plain', p.id as string);
                      e.dataTransfer.effectAllowed = 'move';
                    }}
                    onDragOver={(e) => e.preventDefault()}
                    onDrop={(e) => {
                      e.preventDefault();
                      const sourceId = e.dataTransfer.getData('text/plain') as TLPageId;
                      if (!sourceId || sourceId === p.id) return;
                      if (!editor) return;

                      const srcIndex = pages.findIndex((pg) => pg.id === sourceId);
                      const targetIndex = idx;
                      if (srcIndex === -1) return;

                      // Build order without mutating editor yet
                      const updated = [...pages];
                      const [removed] = updated.splice(srcIndex, 1);
                      updated.splice(targetIndex, 0, removed);

                      const beforeIdx = updated[targetIndex - 1]?.index ?? null;
                      const afterIdx = updated[targetIndex + 1]?.index ?? null;
                      const newIndex = getIndexBetween(beforeIdx as any, afterIdx as any);
                      editor.updatePage({ id: sourceId, index: newIndex });
                      refreshPages();
                    }}
                    className={`w-full px-3 py-2 text-sm flex items-center justify-between hover:bg-gray-100 ${p.id === currentPageId ? 'bg-gray-50 font-medium' : ''}`}
                  >
                    <div className="flex items-center gap-2 text-gray-900">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-700 cursor-grab" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M7 4a1 1 0 112 0 1 1 0 01-2 0zM11 4a1 1 0 112 0 1 1 0 01-2 0zM7 10a1 1 0 112 0 1 1 0 01-2 0zM11 10a1 1 0 112 0 1 1 0 01-2 0zM7 16a1 1 0 112 0 1 1 0 01-2 0zM11 16a1 1 0 112 0 1 1 0 01-2 0z" />
                      </svg>
                      {editingPageId === p.id ? (
                        <input
                          className="truncate text-sm border border-gray-300 rounded px-2 py-1 w-28"
                          value={editingName}
                          autoFocus
                          onChange={(e) => setEditingName(e.target.value)}
                          onBlur={() => {
                            if (editingName.trim() && editor) {
                              editor.updatePage({ id: p.id, name: editingName.trim() });
                              refreshPages();
                            }
                        ...
                        <button onClick={() => handleSwitchPage(p.id)} className="truncate text-left text-gray-900">
                          {p.name}
                        </button>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          const remaining = editor.getPages();
                      ...
                      {/* no blue check; highlight via row bg */}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Dropdown arrow - AFTER undo/redo */}
        <button
        ...
        {/* Main dropdown */}
          {showDropdown && (
          <div ref={dropdownRef} className="absolute left-0 top-12 w-56 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200">
            {/* Share link (for room-based collaboration) */}
            {onCopyShareLink && (
              <>
                <div className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-sm text-gray-700 flex items-center"
